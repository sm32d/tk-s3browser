...
class S3Manager:
    ...
    def load_aws_profiles(self):
        ...
        try:
            ...
            if not self.profiles:
                messagebox.showinfo("No Profiles Found", 
    ...
    def setup_gui(self):
        ...
        self.bucket_combo.grid(row=0, column=1, sticky=(tk.W, tk.E), padx=(0, 5))
        self.bucket_combo.bind('<<ComboboxSelected>>', self.on_bucket_selected)
        
        ttk.Button(bucket_frame, text="Refresh Buckets", command=self.refresh_buckets).grid(row=0, column=2, padx=(5, 0))
        
        # Current path
        self.path_var = tk.StringVar(value="")
        ttk.Label(bucket_frame, text="Path:").grid(row=1, column=0, padx=(0, 5), pady=(5, 0))
        path_entry = ttk.Entry(bucket_frame, textvariable=self.path_var, state="readonly")
        path_entry.grid(row=1, column=1, columnspan=2, sticky=(tk.W, tk.E), pady=(5, 0))
        ...
        ttk.Button(action_frame, text="Up", command=self.go_up).pack(side=tk.LEFT, padx=(0, 5))
    ...
    def open_aws_config(self):
        ...
        try:
            ...
            messagebox.showerror("Error", f"Failed to open AWS config directory: {str(e)}")
    
    def add_profile(self):
        """Show information about adding profiles"""
        messagebox.showinfo("Add AWS Profile", 
            "To add AWS profiles, use the AWS CLI command:\n\n"
            "aws configure --profile <profile-name>\n\n"
            "Or manually edit the files:\n"
            f"• {self.credentials_file}\n"
            f"• {self.config_file}\n\n"
            "Then click 'Refresh Profiles' to reload.")
    
    def delete_profile(self):
        """Show information about deleting profiles"""
        messagebox.showinfo("Delete AWS Profile", 
            "To delete AWS profiles, manually edit the files:\n\n"
            f"• {self.credentials_file}\n"
            f"• {self.config_file}\n\n"
            "Remove the profile section and click 'Refresh Profiles'.")
    ...
    def refresh_buckets(self):
        ...
        try:
            ...
            messagebox.showerror("Error", f"Failed to refresh buckets: {str(e)}")
    ...
    def on_item_double_click(self, event):
        ...
        if len(values) > 1 and values[1] == 'Folder':
            # Navigate to folder
    ...
    def upload_file(self):
        ...
        try:
            ...
            progress_dialog = ProgressDialog(self.root, f"Uploading {file_name}...")
            
            def upload_thread():
                try:
                    self.s3_client.upload_file(file_path, self.current_bucket, s3_key)
                    self.root.after(0, lambda: [progress_dialog.destroy(), 
                                              messagebox.showinfo("Success", f"File uploaded: {file_name}"),
                                              self.refresh_current_folder()])
                except Exception as e:
                    self.root.after(0, lambda: [progress_dialog.destroy(),
    ...
    def download_file(self):
        ...
        try:
            progress_dialog = ProgressDialog(self.root, f"Downloading {file_name}...")
            
            def download_thread():
                try:
                    self.s3_client.download_file(self.current_bucket, s3_key, save_path)
                    self.root.after(0, lambda: [progress_dialog.destroy(),
                                              messagebox.showinfo("Success", f"File downloaded: {file_name}")])
                except Exception as e:
                    self.root.after(0, lambda: [progress_dialog.destroy(),
    ...
    def delete_item(self):
        ...
        try:
            ...
            messagebox.showinfo("Success", f"{item_type} deleted: {item_name}")
            self.refresh_current_folder()
